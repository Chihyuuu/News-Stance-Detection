...
# feature_LDACS
X3 = pd.DataFrame.from_dict(LDA_CS_train,orient='index',columns=['LDA_CS']) # (49972 x 1)
X4 = pd.DataFrame.from_dict(LDA_CS_test,orient='index',columns=['LDA_CS']) # (25413 x 1)
X_LDACS = pd.concat([X3,X4],axis=0,ignore_index=True)
# print (X_LDACS) #(75385 rows x 1 columns)
del X3,X4

#feature_TFIDF+LDACS
X = pd.concat([X,X_LDACS],axis=1)
